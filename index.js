...
googleProvider.setCustomParameters({ prompt: "select_account" });
    ...
    function evaluateMasterRequiredQty(kind, item, line, finishedGood, options) {
      if (!finishedGood || !item || !item.requiredQtyFormulaId) return null;
      const formula = getFormula(item.requiredQtyFormulaId);
      if (!formula || !formula.isActive || !formula.expression) return null;
      const useEnteredDimensions = Boolean(options && options.useEnteredDimensions);
      const dimensionId = useEnteredDimensions ? null : (line && line.dimensionId);
      const variables = kind === "service"
        ? buildServiceFormulaVariables(finishedGood, item, dimensionId, line, formula)
        : buildFormulaVariables(
          finishedGood,
          item,
          Number(line && line.wastagePercent) || 0,
          dimensionId,
          formula,
          line
        );
      ...
      return {
        kind,
        item,
        material: kind === "material" ? item : null,
        service: kind === "service" ? item : null,
        line,
        formula,
        variables,
        finishedGood,
    ...
    function scaledMasterRequiredQty(evaluation, finishedGood) {
      if (!evaluation || evaluation.result === null) return null;
      const factor = isCostCalculatorFinishedGood(finishedGood) ? 1 : bomOrderQuantityValue();
    ...
    function renderBomRequiredQtyDisplay(kind, item, line, finishedGood) {
    ...
    function buildFlatStyleFormulaVariables(finishedGood, plyOverride) {
    ...
    function buildStyleFormulaResultRow(formula, calculated, linkId, ply) {
      return {
        linkId: linkId,
        formulaId: formula.id,
    ...
    function getStyleFormulaMetrics(finishedGood, evaluatedStyleFormulas) {
      ...
      const productPly = getFinishedGoodPly(finishedGood);
      const coveredRows = rows.filter((row) => isCoveredAreaStyleFormula(row));
      const coveredRow = coveredRows.find((row) => Number(row.ply) === productPly)
        || coveredRows.find((row) => row.ply == null)
        || coveredRows[0]
        || null;
      return {
        rows,
    ...
    function coveredAreaRowForPly(snap, ply) {
    ...
    function evaluateCoveredAreaValue(finishedGood, fallbackVars, metrics, plyOverride) {
      const snap = metrics || getStyleFormulaMetrics(finishedGood);
      const ply = plyOverride != null ? Number(plyOverride) : getFinishedGoodPly(finishedGood);
      const plyRow = coveredAreaRowForPly(snap, ply);
      if (plyRow) {
        const n = plyRow.success ? numericOrNull(plyRow.result) : null;
        return {
          success: n !== null,
          result: n,
      ...
      if (style && styleHasPlyCoveredAreaFormulas(style.id) && isStylePly(ply)) {
        return {
          success: false,
          result: null,
          error: "No Covered Area formula linked for ply " + ply + ".",
          source: "style_covered_area",
          ply
        };
      }
      const vars = fallbackVars || buildFlatStyleFormulaVariables(finishedGood, ply);
      ...
      if (!areaFormula || !areaFormula.isActive) {
        return { success: false, result: null, error: "COVERED_AREA formula is not available.", source: "covered_area", ply };
      }
    ...
    function injectStyleFormulaResults(finishedGood, variables, plyOverride) {
      ...
      if (covered.success) {
        const n = numericOrNull(covered.result);
    ...
    function appendHtmlUnit(text, uom, separator) {
      const unit = String(uom || "").trim();
      if (!unit || text == null || text === "" || text === "—") return text;
      return text + separator + escapeHtml(unit);
    }

    function appendPlainUnit(text, uom, separator) {
      const unit = String(uom || "").trim();
      if (!unit || text == null || text === "" || text === "—") return text;
      return text + separator + unit;
    }

    function renderCompactLW(L, W, uom) {
      const length = Number(L);
      const width = Number(W);
      if (!Number.isFinite(length) || !Number.isFinite(width) || length <= 0 || width <= 0) return "";
      const suffix = String(uom || "").trim() ? " " + escapeHtml(String(uom).trim()) : "";
      return `L = ${escapeHtml(formatQty(length))}${suffix}, W = ${escapeHtml(formatQty(width))}${suffix}`;
    }

    function formatBreakdownMaterialDimensions(finishedGood, line, uom) {
      if (isUseCustomDimensions(line)) {
        const custom = renderCompactLW(line.customLength, line.customWidth, uom);
        if (custom) return custom;
      }
      const auto = getAutoQuantityLW(finishedGood, line && line.dimensionId, styleFormulaPlyForBomLine(finishedGood, line));
      return (auto && renderCompactLW(auto.L, auto.W, uom)) || "—";
    }

    function renderStepTableDimCell(autoDims, customL, customW, uom) {
      const autoLine = autoDims ? renderCompactLW(autoDims.L, autoDims.W, uom) : "";
      const customLine = renderCompactLW(customL, customW, uom);
    ...
    function getBomLineModalPanel() {
      return (state.modal && state.modal.panel) || "full";
    }

    function renderStepValueWithEdit(valueHtml, dataAttr, dataValue, title, icon, tone) {
      const toneClass = tone ? ` btn-icon-${escapeHtml(tone)}` : "";
      return `
        <div class="step-inline-edit">
          <span class="step-inline-edit-value">${valueHtml}</span>
          <button type="button" class="btn btn-sm btn-icon${toneClass}" ${dataAttr}="${escapeHtml(String(dataValue))}" title="${escapeHtml(title)}" aria-label="${escapeHtml(title)}">
            <i data-lucide="${escapeHtml(icon || "pencil")}"></i>
          </button>
        </div>
      `;
    }
    ...
    function renderFinishingDimensionCell(finishedGood, line, dataAttr) {
      ...
      return renderStepValueWithEdit(
        formatBreakdownMaterialDimensions(finishedGood, line, finishedGood && finishedGood.dimensionUOM),
    ...
    function renderRequiredQtyWithEdit(valueHtml, dataAttr, lineId) {
      if (lineId == null || lineId === "") return valueHtml;
      return renderStepValueWithEdit(valueHtml, dataAttr, lineId, "Edit required qty", "pencil", "edit");
    }
    ...
    function styleFormulaPlyForLayer(layer, productPly) {
      const ply = normalizeStylePly(productPly, 3);
      const layers = getStructuralLayers(ply);
      const name = canonicalPlyLayerName(layer);
      const index = layers.indexOf(name);
      if (index < 0) return ply;
      return index + 1;
    }

    function styleFormulaPlyForBomLine(finishedGood, line) {
      const productPly = getFinishedGoodPly(finishedGood);
      if (!line || isBomAccessoryLine(line)) return productPly;
      return styleFormulaPlyForLayer(line.layer, productPly);
    }
    ...
    function isAccessoryRawMaterial(rawMaterialId) {
      const material = getRawMaterial(rawMaterialId);
      return Boolean(material && material.category === "Accessories");
    }
    ...
    function isBomAccessoryLine(line) {
      if (!line) return false;
      return String(line.layer || "") === "Accessories" || isAccessoryRawMaterial(line.rawMaterialId);
    }
    ...
    function normalizeServicePlies(value) {
      const raw = Array.isArray(value) ? value : (value != null && value !== "" ? [value] : []);
      const seen = new Set();
      const next = [];
      raw.forEach((item) => {
    ...
    function serviceMatchesPly(service, ply) {
      if (!service) return false;
      const linked = normalizeServicePlies(service.plies);
      if (!linked.length) return true;
      return linked.includes(Number(ply));
    }

    function servicesForPlySlot(categoryId, ply, selectedId) {
      const options = services.filter((item) => item.status !== "Inactive" && serviceHasCategory(item, categoryId) && serviceMatchesPly(item, ply));
    ...
    function buildFormulaVariables(finishedGood, material, wastagePercent, dimensionId, formula, line) {
      ...
      const styleVals = {};
      const style = finishedGood ? findStyleByName(finishedGood?.style) : null;
      const layerPly = styleFormulaPlyForBomLine(finishedGood, line);
      if (style) {
        getStyleVariablesForPly(style.id, layerPly).forEach((row) => {
          const n = numericOrNull(row.value);
    ...
    function calculateMaterialCost(materialLine, context) {
      ...
      if (line.calculationMethod === "manual") {
        ...
        const style = findStyleByName(finishedGood.style);
        const layerPly = styleFormulaPlyForBomLine(finishedGood, line);
        if (style && styleHasPlyCoveredAreaFormulas(style.id)) {
          const areaEval = evaluateCoveredAreaValue(finishedGood, variables, null, layerPly);
          if (!areaEval.success) {
            line.error = areaEval.error || ("No Covered Area formula linked for ply " + layerPly + ".");
            line.netQty = 0;
            line.grossQty = 0;
            line.costPerPiece = 0;
            return line;
    ...
    function recalculateBOMCosts() {
      ...
      state.bomFinishingServices = (state.bomFinishingServices || []).map((line) => {
        if (!line || !line.serviceId) return { ...line, error: null, quantity: 0, rate: 0, costPerPiece: 0 };
        return calculateServiceCost(line);
      });
    ...
    function renderStyleFormulaRow(row, options) {
      ...
      return `
        <div class="${classes.join(" ")}">
          <div>
            <div>${stepLabel}<span class="mono">${escapeHtml(plyLabel + (row.code || ""))}</span>: ${escapeHtml(row.description || row.name)}</div>
    ...
    function renderStyleFormulaResultRows(rows) {
      ...
      coveredRows.forEach((covered) => {
        coveredAreaDependentStyleRows(covered, list).forEach((row) => nestedKeys.add(String(row.linkId || row.code)));
      });
      const coveredKeys = new Set(coveredRows.map((row) => String(row.linkId || row.code)));
      ...
      const coveredHtml = coveredRows.map((row) => renderStyleFormulaRow(row, { primary: true })).join("");
      return `
        <div class="style-formula-results">
          ${independentHtml}
          ${coveredHtml}
    ...
    function getCostCalculatorMaterials() {
      return rawMaterials.filter((item) => item.status !== "Inactive" && item.category !== "Accessories");
    }
    ...
    function serviceLineHasStructuralLayer(row, plyCount) {
      const name = canonicalPlyLayerName(row && row.layer);
      if (!name) return false;
      return getStructuralLayers(plyCount).includes(name);
    }

    function layerServiceLines(lines, finishedGood, makeId) {
      ...
      return layers.map((layer) => {
        ...
        let line = prev.find((row) => row && canonicalPlyLayerName(row.layer) === layer && !used.has(row.id));
        if (!line) {
          const productPly = finishedGood && finishedGood.ply;
          line = prev.find((row) => row && row.serviceId && !used.has(row.id) && !serviceLineHasStructuralLayer(row, productPly) && serviceMatchesPly(getService(row.serviceId), ply));
        }
        if (line) {
          used.add(line.id);
          return { ...line, layer, ply };
        }
        return {
          id: makeId(),
          layer,
    ...
    function syncCostCalculatorServiceSlots(listKey) {
      const steps = getCostCalculatorStepState();
      if (!steps.hasPly) return false;
      const layers = getStructuralLayers(steps.ply);
      const prev = Array.isArray(state.costCalculator[listKey]) ? state.costCalculator[listKey] : [];
      const used = new Set();
      let changed = prev.length !== layers.length;
      const next = layers.map((layer) => {
        const ply = styleFormulaPlyForLayer(layer, steps.ply);
        let existing = prev.find((row) => row && row.layer === layer && !used.has(row.id || row.key));
        if (!existing) {
          existing = prev.find((row) => row && row.serviceId && !used.has(row.id || row.key) && !serviceLineHasStructuralLayer(row, steps.ply) && serviceMatchesPly(getService(row.serviceId), ply));
          changed = true;
        }
        if (existing) {
          used.add(existing.id || existing.key);
        ...
        changed = true;
        const key = nextCostCalculatorServiceKey();
        return { id: key, key, layer, serviceId: "" };
      });
      if (!changed) return false;
      state.costCalculator[listKey] = next;
      return true;
    }
    ...
    function normalizeCostCalculatorServiceRow(row) {
      ...
      return {
        id,
        key: id,
        layer: row && row.layer ? canonicalPlyLayerName(String(row.layer)) : "",
        serviceId,
    ...
    function renderCostCalculatorServices(summary, steps) {
      ...
      const active = (summary.services || []).filter((row) => row.serviceId);
      const hasServiceErrors = active.some((row) => row.calc && row.calc.error);
      return `
        <div class="table-wrap">
          <table class="data-table step-grid-table">
            <thead>
    ...
    function renderCostCalculator() {
      ...
      const finishingRows = renderCostCalculatorFinishingServices(summary, steps);
    ...
    function getBomMaterialSlotLayout(finishedGood, materials) {
      ...
      rows.forEach((item) => {
        ...
        if (isBomAccessoryLine(item)) return;
    ...
    function getBomSlotMaterialOptions(ply, selectedId) {
      const options = getCostCalculatorMaterials(ply).filter((item) => item.category !== "Accessories");
      const id = Number(selectedId);
      if (id && !options.some((item) => item.id === id)) {
        const current = getRawMaterial(id);
        if (current) options.unshift(current);
    ...
    function assignBomLayerService(kind, layer, serviceId) {
      ...
      if (!fg) return;
      const key = kind === "finishing" ? "bomFinishingServices" : "bomServices";
      syncBomServiceSlots(kind);
      const line = (state[key] || []).find((item) => item.layer === layer);
      if (!line) return;
      const value = serviceId ? Number(serviceId) : null;
      if (!value) {
        state[key] = state[key].map((item) => item.id === line.id ? {
          ...item,
          serviceId: null,
          error: null,
          quantity: 0,
      ...
      if (!service) return;
      const fields = defaultGeneralServiceFields(service, fg);
      const next = calculateServiceCost({
        ...line,
        ...fields,
        layer,
        ply: styleFormulaPlyForLayer(layer, fg.ply),
        quantity: 0,
        rate: 0,
        costPerPiece: 0
      });
      state[key] = state[key].map((item) => item.id === line.id ? next : item);
      recalculateBOMCosts();
      refreshBomViews();
      persistEditorState();
      if (next.error) showNotification(next.error, "error");
      else showNotification((service ? service.name : "Service") + " selected for " + layer);
    }

    function assignCostCalculatorLayerService(listKey, layer, serviceId) {
      syncCostCalculatorServiceSlots(listKey);
      const row = (state.costCalculator[listKey] || []).find((item) => item.layer === layer);
      if (!row) return;
      const steps = getCostCalculatorStepState();
      if (!serviceId) {
        row.serviceId = "";
      ...
      row.ply = styleFormulaPlyForLayer(layer, steps.ply);
      persistCostCalculatorState();
      renderCostCalculator();
      refreshIcons();
      const calc = calculateCostCalculatorService(listKey === "finishingServices" ? { ...row, finishing: true } : row);
    ...
    function saveCostCalculatorAsBom() {
      ...
      state.bomServices = summary.services.filter((row) => row.serviceId).map((row) => calculateServiceCost({
        id: nextBomLineId(),
      ...
      state.bomFinishingServices = (summary.finishingServices || []).filter((row) => row.serviceId).map((row) => calculateServiceCost({
        id: nextBomLineId(),
        serviceId: Number(row.serviceId),
        layer: row.layer,
        ply: styleFormulaPlyForLayer(row.layer, fg.ply),
        ...
        ? summary.layers.map((row) => {
            ...
            return `
              <tr>
                ...
                <td class="num">${row.rawMaterialId ? escapeHtml(formatMasterRequiredQty("material", material, { layer: row.layer, rawMaterialId: row.rawMaterialId, wastagePercent: calc.wastagePercent }, getCostCalculatorFinishedGood(), { useEnteredDimensions: true })) : "—"}</td>
        ...
        ? (summary.additionalMaterials || []).map((row) => {
            ...
            return `
              <tr>
                ...
                <td class="num">${escapeHtml(formatMasterRequiredQty("material", material, row, getCostCalculatorFinishedGood(), { useEnteredDimensions: true }))}</td>
                <td class="num">${ok ? escapeHtml(formatRatePkr(calc.rate, calc.rateUOM || (getMaterialRate(material && material.id) && getMaterialRate(material && material.id).rateUOM))) : "—"}</td>
            ...
            return `
              <tr>
                ...
                <td class="num">${escapeHtml(formatMasterRequiredQty("service", service, row, getCostCalculatorFinishedGood(), { useEnteredDimensions: true }))}</td>
    ...
    function handleFinishedGoodChange(id) {
      ...
      state.bomAdditionalServices = [];
      state.bomFinishingServices = [];
      syncBomServiceSlots("services");
      syncBomServiceSlots("finishing");
    ...
    function styleFormulasPopupDisplayModel(rows) {
      ...
      if (coveredRows.length) {
        coveredGroup = {
          hasNested: false,
          rows: coveredRows.map((row) => styleFormulaRowDisplayModel(row, { primary: true }))
        };
      }
    ...
    function bomLineCostDisplayModel(line) {
      ...
      return {
        hasError,
        displayValue: hasError ? "Error" : appendPlainUnit(formatRupees(line.costPerPiece), fg && fg.uom, " / ")
      };
    }
    ...
    function renderBomMaterialCard(options) {
      ...
      const accessory = Boolean(options.accessory);
      ...
      const materials = accessory
        ? getAccessoryRawMaterials(line && line.rawMaterialId)
        : extra
          ? getBomExtraMaterialOptions(line && line.rawMaterialId)
          : getBomSlotMaterialOptions(ply, line && line.rawMaterialId);
      const selectId = accessory
        ? "bom-accessory-mat-" + (line && line.id)
        : extra ? "bom-extra-mat-" + line.id : "bom-layer-mat-" + index;
      const selectAttr = accessory
        ? "disabled"
        : extra
      ...
      return `
        <div class="cc-layer step-liner-block">
          <div class="step-liner-head">
            <div class="step-liner-title">${escapeHtml(layer)} Material${extra ? ` <span class="badge badge-muted">Extra</span>` : ""}${accessory ? ` <span class="badge badge-muted">From product</span>` : ""}</div>
            <div class="step-liner-material">${escapeHtml(materialTitle)}</div>
          </div>
                  ...
                  <tr>
                    ...
                    <td class="step-col-actions">
                      <div class="row-actions">
                        ...
                        <button type="button" class="btn btn-sm btn-icon btn-icon-edit" data-edit-line="${line.id}" title="Edit calculation details">
                          <i data-lucide="pencil"></i>
                        </button>
                        ${extra && !accessory ? `
      ...
      return `
        <div class="cc-layer">
          <div class="cc-layer-head">
            ...
            ${line ? `
              <div class="row-actions">
                <button type="button" class="btn btn-sm btn-icon btn-icon-calc" data-breakdown-other-line="${line.id}" title="Calculation breakdown">
                  <i data-lucide="calculator"></i>
                </button>
                <button type="button" class="btn btn-sm btn-icon btn-icon-edit" data-edit-other-line="${line.id}" title="Edit calculation details">
                  <i data-lucide="pencil"></i>
                </button>
                ${extra ? `
      ...
      const extraBody = extraRows.map((line, index) => {
        ...
        return `
          <tr>
            ...
            <td class="step-num">${renderRequiredQtyWithEdit(renderBomRequiredQtyDisplay("material", material, line, getSelectedFinishedGood()), "data-edit-required-qty-line", line.id)}</td>
            ...
            <td class="step-num" data-bom-line-cost="additional-material:${line.id}">${line.error ? `<span class="calc-error-cost">Error</span>` : appendHtmlUnit(formatRupees(line.costPerPiece), (getSelectedFinishedGood() || {}).uom, " / ")}</td>
            <td>
              <div class="row-actions">
                <button type="button" class="btn btn-sm btn-icon btn-icon-calc" data-breakdown-line="${line.id}" title="Calculation breakdown">
                  <i data-lucide="calculator"></i>
                </button>
                <button type="button" class="btn btn-sm btn-icon btn-icon-edit" data-edit-line="${line.id}" title="Edit">
                  <i data-lucide="pencil"></i>
                </button>
                <button type="button" class="btn btn-sm btn-icon btn-danger" data-delete-line="${line.id}" title="Delete">
                  <i data-lucide="trash-2"></i>
      ...
      const serviceBody = additionalRows.map((line, index) => {
        ...
        return `
          <tr>
            ...
            <td class="step-num">${renderRequiredQtyWithEdit(renderBomRequiredQtyDisplay("service", service, line, getSelectedFinishedGood()), "data-edit-required-qty-additional", line.id)}</td>
            ...
            <td class="step-num" data-bom-line-cost="additional-service:${line.id}">${line.error ? `<span class="calc-error-cost">Error</span>` : appendHtmlUnit(formatRupees(line.costPerPiece), (getSelectedFinishedGood() || {}).uom, " / ")}</td>
            <td>
              <div class="row-actions">
                <button type="button" class="btn btn-sm btn-icon btn-icon-calc" data-breakdown-additional-service="${line.id}" title="Calculation breakdown">
                  <i data-lucide="calculator"></i>
                </button>
                <button type="button" class="btn btn-sm btn-icon btn-icon-edit" data-edit-additional-service="${line.id}" title="Edit">
                  <i data-lucide="pencil"></i>
                </button>
                <button type="button" class="btn btn-sm btn-icon btn-danger" data-delete-additional-service="${line.id}" title="Delete">
                  <i data-lucide="trash-2"></i>
    ...
    function renderFinishingServicesSection() {
      ...
      const body = rows.map((line, index) => {
        ...
        return `
          <tr>
            ...
            <td class="step-num step-col-compact">${renderBomRequiredQtyDisplay("service", service, line, fg)}</td>
            ...
            <td class="step-num step-col-compact" data-bom-line-cost="finishing:${line.id}">${line.error ? `<span class="calc-error-cost">Error</span>` : appendHtmlUnit(formatRupees(line.costPerPiece), (fg || {}).uom, " / ")}</td>
            <td>
              <div class="row-actions">
      ...
      mountBomCostCellHosts(
        root,
        lineModels,
        activeRows.length ? "bom-finishing-cost-total" : null,
        activeRows.length ? bomSectionTotalDisplayModel(hasFinishingErrors, state.totalFinishingServiceCost) : null
      );
    }
      ...
      const infoForm = `
          <div class="form-grid">
            ...
            <div class="form-span-2">
              ...
              <p class="stat-hint" style="margin:4px 0 0;">Link the plies this service can be selected on. If none are checked, it stays available on every ply.</p>
            </div>
          </div>
      `;
    ...
    function renderServiceRateFormModal() {
      ...
      if (state.modal.compact) {
        return `
          <div class="modal-header">
            <div>
              <div class="section-kicker">Service rate</div>
              <strong>Edit Rate</strong>
            </div>
            <button type="button" class="btn btn-ghost btn-sm" data-modal-close>Close</button>
          </div>
    ...
    function buildFormulaExplainModel_OrderQty() {
      ...
      const steps = entries.map((entry, index) => {
        ...
        const resultHtml = entry.error
          ...
          : `<div>Formula result: <strong class="formula-val">${escapeHtml(formatExplainNumber(entry.result))}</strong></div>`;
        return {
          index: index + 1,
    ...
    function findBomRequiredQtyTarget(lineId) {
      const id = Number(lineId);
      const materialLine = (state.bomMaterials || []).find((line) => line && line.id === id);
      if (materialLine) {
        return { kind: "material", line: materialLine, item: getRawMaterial(materialLine.rawMaterialId) };
      }
      const serviceLists = [
        state.bomServices,
        state.bomAdditionalServices,
      ...
      for (let i = 0; i < serviceLists.length; i += 1) {
        const serviceLine = (serviceLists[i] || []).find((line) => line && line.id === id);
        if (serviceLine) {
          return { kind: "service", line: serviceLine, item: getService(serviceLine.serviceId) };
        }
      }
      return null;
    }

    function buildFormulaExplainModel_RequiredQty(lineId) {
      const target = findBomRequiredQtyTarget(lineId);
      const fg = getSelectedFinishedGood();
      ...
      if (!evaluation) {
        return { error: "No Default Required Quantity Formula is set on this " + (target.kind === "service" ? "service." : "raw material.") };
      }
      const orderQty = bomOrderQuantityValue();
      const scaled = scaledMasterRequiredQty(evaluation, fg);
      const inputs = orderQtyExplainInputs(evaluation);
      const inputHtml = inputs.map((input) =>
        `<div><strong>${escapeHtml(input.code)}</strong> — ${escapeHtml(input.origin)}: <strong class="formula-val">${escapeHtml(input.shown)}</strong></div>`
      ).join("");
      const formulaResultHtml = evaluation.error
        ? `<div>${escapeHtml(evaluation.error)}</div>`
        : `<div>Formula result: <strong class="formula-val">${escapeHtml(formatExplainNumber(evaluation.result))}</strong></div>`;
      const multiplyHtml = scaled === null
        ? `<div>Required Qty could not be calculated.</div>`
        : `<div>${escapeHtml(formatExplainNumber(evaluation.result))} × Order Quantity ${escapeHtml(formatExplainNumber(orderQty))} = <strong class="formula-val">${escapeHtml(formatExplainNumber(scaled))}</strong></div>`;
      return {
        title: "Required Qty",
        subtitle: (target.item.name || target.item.code || "") + " · Default Required Quantity Formula",
        formulaName: evaluation.formula.name || evaluation.formula.code,
        sourceType: "formula",
        summaryHtml: "Required Qty is the Default Required Quantity Formula result multiplied by the Order Quantity entered in the cost summary.",
        steps: [
          {
            index: 1,
            heading: (evaluation.formula.name || evaluation.formula.code) + " (" + evaluation.formula.code + ")",
            expression: prettyExpressionText(evaluation.formula.expression),
            pluggedHtml: inputHtml + formulaResultHtml
          },
          {
            index: 2,
            heading: "Required Qty",
            expression: "Formula result × Order Quantity",
            pluggedHtml: multiplyHtml
          }
        ],
        finalHtml: scaled === null
          ? "Required Qty could not be calculated."
          : `Required Qty: <strong>${escapeHtml(formatQty(scaled))}</strong>`
      };
    }
    ...
    function renderCompactLineModal(title, bodyHtml, saveId, kicker) {
      return `
        <div class="modal-header">
          <div>
            <div class="section-kicker">${escapeHtml(kicker || "BOM line")}</div>
        ...
        <div class="modal-body compact-line-body">${bodyHtml}</div>
        <div class="modal-footer">
          <button type="button" class="btn" data-modal-close>Cancel</button>
          <button type="button" class="btn btn-primary" id="${escapeHtml(saveId)}">Save</button>
        </div>
      `;
    }

    function renderMaterialDimsPopup() {
      const draft = state.modal.draft;
      const errors = state.modal.errors || {};
      const material = getRawMaterial(draft.rawMaterialId);
      ...
      const widthValue = draft.customWidth == null || draft.customWidth === "" ? "" : String(draft.customWidth);
      const body = `
        <p class="stat-hint" style="margin:0 0 12px;">${material ? escapeHtml(material.name) + " — override length and width used in quantity formulas." : "Override length and width used in quantity formulas."}</p>
        <div class="dim-popup-card">
          ...
          <label class="custom-dim-flag dim-popup-toggle">
            <input id="modal-material-use-custom-dim" type="checkbox" ${checked ? "checked" : ""} />
            <span>
              <strong>Use Custom Dimensions</strong>
              <span class="stat-hint">Replace the current length and width for this material line</span>
            </span>
          </label>
          ${checked ? `
            <div class="dim-input-row two dim-popup-fields">
              <div>
                <label class="form-label" for="modal-material-custom-length">Length (L)</label>
                <div class="input-with-unit ${errors.customLength ? "is-invalid" : ""}">
                  <input id="modal-material-custom-length" type="number" step="any" min="0.0001" value="${escapeHtml(lengthValue)}" aria-invalid="${errors.customLength ? "true" : "false"}" />
                  <span>in.</span>
                </div>
                ${errors.customLength ? `<div class="field-error">${escapeHtml(errors.customLength)}</div>` : ""}
              </div>
              <div>
                <label class="form-label" for="modal-material-custom-width">Width (W)</label>
                <div class="input-with-unit ${errors.customWidth ? "is-invalid" : ""}">
                  <input id="modal-material-custom-width" type="number" step="any" min="0.0001" value="${escapeHtml(widthValue)}" aria-invalid="${errors.customWidth ? "true" : "false"}" />
                  <span>in.</span>
                </div>
                ${errors.customWidth ? `<div class="field-error">${escapeHtml(errors.customWidth)}</div>` : ""}
              </div>
    ...
    function renderMaterialQtyMethodFields(draft, errors, qtyLabel) {
      ...
      return `
        ...
        ` : renderManualQtyAndRateFields("modal-manual-qty", "modal-manual-rate", draft, errors, qtyLabel || "Manual quantity")}
      `;
    }

    function renderMaterialQtyPopup() {
      const draft = state.modal.draft;
      const errors = state.modal.errors || {};
      const material = getRawMaterial(draft.rawMaterialId);
      const body = `
        <p class="stat-hint" style="margin:0 0 12px;">${material ? escapeHtml(material.name) + " (" + escapeHtml(material.code) + ")" : "Material not found"}</p>
        <div class="form-grid">
          ${renderMaterialQtyMethodFields(draft, errors, "Quantity / Piece")}
        </div>
        ...
        ${errors.customLength ? `<div class="field-error" style="margin-top:10px;">${escapeHtml(errors.customLength)}</div>` : ""}
        ${errors.customWidth ? `<div class="field-error" style="margin-top:10px;">${escapeHtml(errors.customWidth)}</div>` : ""}
      `;
      ...
      return `
        ...
        <div class="modal-body">
          <div class="detail-list">
            ...
            <div><span>Required Qty</span><strong>${formatMasterRequiredQty("material", material, line, fg, fromCalculator ? { useEnteredDimensions: true } : null)}</strong></div>
    ...
    function openMaterialModal(lineId, panel) {
      ...
      const slotLocked = layout.slots.some((slot) => slot.line && slot.line.id === line.id) || isBomAccessoryLine(line);
    ...
    function openDeleteMaterialModal(lineId) {
      const fromCalculator = Boolean(findCostCalculatorAdditionalMaterialById(lineId));
      const bomLine = state.bomMaterials.find((item) => item.id === Number(lineId));
      if (!fromCalculator && isBomAccessoryLine(bomLine)) {
        showNotification("Change this accessory on the finished good, not on the BOM.", "error");
        return;
      }
      state.modal = {
        type: "confirm-delete",
    ...
    function confirmDeleteMaterial() {
      ...
      const deleting = state.bomMaterials.find((line) => line.id === Number(state.modal.lineId));
      if (isBomAccessoryLine(deleting)) {
        closeModal();
        showNotification("Change this accessory on the finished good, not on the BOM.", "error");
        return;
      }
    ...
    function renderServiceQtyPopup() {
      const draft = state.modal.draft;
      const errors = state.modal.errors || {};
      const service = getService(draft.serviceId);
      const body = `
        <p class="stat-hint" style="margin:0 0 12px;">${service ? escapeHtml(service.name) + " (" + escapeHtml(service.code) + ")" : "Service not found"}</p>
        <div class="form-grid">
          ${renderServiceQtyMethodFields(draft, errors)}
        </div>
        ${errors.duplicate ? `<div class="field-error" style="margin-top:10px;">${escapeHtml(errors.duplicate)}</div>` : ""}
        ${errors.finishedGood ? `<div class="field-error" style="margin-top:10px;">${escapeHtml(errors.finishedGood)}</div>` : ""}
        ${errors.rate ? `<div class="field-error" style="margin-top:10px;">${escapeHtml(errors.rate)}</div>` : ""}
    ...
    function openServiceModal(lineId, collection, panel) {
    ...
    function saveServiceFromModal() {
      ...
      if (editing) {
        if (isCostCalculatorFinishingModal()) existingLine = findCostCalculatorFinishingServiceLineById(lineId);
        else if (isCostCalculatorAdditionalServiceModal()) existingLine = findCostCalculatorAdditionalServiceById(lineId);
        else if (isCostCalculatorServiceModal()) existingLine = findCostCalculatorServiceLineById(lineId);
        else if (isAdditional) existingLine = (state.bomAdditionalServices || []).find((line) => line.id === lineId) || null;
        else if (isFinishing) existingLine = (state.bomFinishingServices || []).find((line) => line.id === lineId) || null;
        else existingLine = (state.bomServices || []).find((line) => line.id === lineId) || null;
      }
    ...
    function setupEventHandlers() {
      ...
      document.querySelector(".content").addEventListener("change", (event) => {
        ...
        if (bomServiceSlot) {
          assignBomLayerService("services", bomServiceSlot, event.target.value);
          return;
        }
        const bomFinishingSlot = event.target.dataset && event.target.dataset.bomFinishingSlot;
        if (bomFinishingSlot) {
          assignBomLayerService("finishing", bomFinishingSlot, event.target.value);
          return;
        }
        const ccServiceSlot = event.target.dataset && event.target.dataset.ccServiceSlot;
        if (ccServiceSlot) {
          assignCostCalculatorLayerService("services", ccServiceSlot, event.target.value);
          return;
        }
        const ccFinishingSlot = event.target.dataset && event.target.dataset.ccFinishingSlot;
        if (ccFinishingSlot) {
          assignCostCalculatorLayerService("finishingServices", ccFinishingSlot, event.target.value);
          return;
        }
      ...
      document.querySelector(".content").addEventListener("click", (event) => {
        ...
        const editRequiredQtyLine = event.target.closest("[data-edit-required-qty-line]");
        if (editRequiredQtyLine) {
          openMaterialModal(editRequiredQtyLine.dataset.editRequiredQtyLine, "qty");
          return;
        }
        const editRequiredQtyAdditional = event.target.closest("[data-edit-required-qty-additional]");
        if (editRequiredQtyAdditional) {
          const additionalId = editRequiredQtyAdditional.dataset.editRequiredQtyAdditional;
          const additionalCollection = findCostCalculatorAdditionalServiceById(additionalId) ? "cc-additional-service" : "additional";
          openServiceModal(additionalId, additionalCollection, "qty");
          return;
        }
        const editRequiredQtyService = event.target.closest("[data-edit-required-qty-service]");
        if (editRequiredQtyService) {
